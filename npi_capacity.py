...
def get_tool_cavs(df_sql: pd.DataFrame):
    tool_mask = df_sql.apply(lambda x: match_tool_desc(x), axis=1)
...
def get_cycle_times(df_tool: pd.DataFrame, df_mould: pd.DataFrame):
...
def add_quality_days(ordrbk_df: pd.DataFrame):
    ...
    ordrbk_df["extra_docs"] = ordrbk_df["COLExt Text"].apply(
        detect_quality_docs
    )
